...
import { ReviewsService, REVIEW_APPROVED_EVENT, REVIEW_REJECTED_EVENT } from './reviews.service';
...
export class ReviewsHandler {
  ...
  @OnEvent(REVIEW_REJECTED_EVENT)
  async handleReviewRejected(payload: { productId: string }): Promise<void> {
    await this.reviewsService.recomputeRating(payload.productId);
  }
}
