...
async function bootstrap(): Promise<void> {
  ...
  expressApp.use(createProxyMiddleware({
    ...
    on: {
      error: (_err, _req, res) => {
        (res as Response).status(502).json({ statusCode: 502, message: 'Auth service unavailable' });
      },
    },
  }));
  ...
  expressApp.use(createProxyMiddleware({
    ...
    on: {
      error: (_err, _req, res) => {
        (res as Response).status(502).json({ statusCode: 502, message: 'Search service unavailable' });
      },
    },
  }));

  // 4. Backend catch-all  (/api/* except auth + search)
  ...
  expressApp.use(createProxyMiddleware({
    ...
    on: {
      error: (_err, _req, res) => {
        (res as Response).status(502).json({ statusCode: 502, message: 'Backend service unavailable' });
      },
    },
  }));
