...
- [x] **Search input debounce** — already tracked in `08-user-experience.md`. The search should not fire on every keystroke. Debounce by 300ms.
  - Complexity: Easy (covered in 08)

- [x] **Search history with `localStorage`** — save the last 5–10 search terms when the user submits a search. Show them as recent searches when the user focuses the search input with an empty value:
  ```ts
...
- [x] **Search suggestions / autocomplete** — as the user types, show a dropdown of matching product names. Options:
...
- [x] **Highlight matched terms in results** — when showing search results, bold/highlight the matched portion of the product name:
  ```tsx
...
- [x] **Empty search results state** — when a search returns 0 results, show a helpful empty state (not a blank page):
...
- [x] **No-query empty state** — when the search input is cleared and the user is back to browse mode with 0 loaded products (edge case), show a "No products found" state with a CTA. Tracked in `33-empty-states-loading.md`.
  - Complexity: Easy

- [x] **Search input keyboard accessibility** — the search input should:
