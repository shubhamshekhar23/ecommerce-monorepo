...
- [x] **Identify largest dependencies** — after running the analyzer, look for:
...
- [x] **Route-level code splitting** — Next.js already splits per route. Verify that no route's page.tsx imports the entire app's logic. Check for barrel file anti-patterns:
  ```ts
...
- [x] **Ensure tree shaking works** — tree shaking eliminates unused code. It only works with ES modules (`import`/`export`), not CommonJS (`require`). Verify:
...
- [x] **Avoid large library defaults** — common mistakes that bloat bundles:
